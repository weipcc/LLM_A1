...
# ------------------------------------------------------------------
# Exercise 4: 串流版本的聊天邏輯
# ------------------------------------------------------------------
def chat_fn(message, history):
    """
    這裡改成 generator（用 yield 而不是 return），
    Gradio 偵測到 function 是 generator 時，會自動把每次 yield 的內容
    更新到畫面上，達成「一段段跑出來」的效果。
    """
    # --- 組合歷史紀錄 ---
    contents = []
    for turn in history:
    ...
    for chunk in response_stream:
        if chunk.text:
            # Gemini 傳回的 chunk 通常是一整段文字，
            # 這裡刻意拆成一個字一個字 yield，並加上小延遲，
            # 讓畫面上呈現更明顯的「打字機」效果。
