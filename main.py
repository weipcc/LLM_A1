...
def chat_fn(message, history):
    ...
    try:
        for chunk in response_stream:
            # --- Diagnostic: inspect per-category safety ratings and
            # finish_reason, so we can tell apart an API-level safety
            # filter block from the model simply choosing to refuse. ---
            candidates = getattr(chunk, "candidates", None)
            if candidates:
                finish_reason = getattr(candidates[0], "finish_reason", None)
                safety_ratings = getattr(candidates[0], "safety_ratings", None)
                if finish_reason and str(finish_reason) != "STOP":
                    print(f"[SAFETY] finish_reason = {finish_reason}")
                if safety_ratings:
                    for rating in safety_ratings:
                        print(f"[SAFETY] {rating.category} -> {rating.probability}")

            # Things to check Q4: if Gemini's safety filters block the
            # response, chunk.text may be empty and the block reason is
            # exposed via prompt_feedback / candidates[0].finish_reason
            # instead of raising an exception.
            if not chunk.text:
                feedback = getattr(chunk, "prompt_feedback", None)
                if feedback and getattr(feedback, "block_reason", None):
                    print(f"\n[SAFETY] Prompt blocked. Reason: {feedback.block_reason}\n")
                    yield (
                        "⚠️ I can't answer that - the request was blocked by "
                        "the safety filters."
                    )
                    return
                continue
        ...
        # Some safety blocks surface as an exception rather than an empty
        # response, depending on the SDK version.
        print(f"\n[SAFETY] Generation stopped unexpectedly: {e}\n")
        yield (
            partial_text
            + "\n\n⚠️ The response was stopped, possibly by the safety filters."
        )
